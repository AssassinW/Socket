...
int main(){
    ...
    if((fp=fopen("client1.txt","r"))==NULL){		//read the file
...
/*The searching loop*/
  ...
  if(name_search!=NULL){
    ...
    printf("The Client 1 sends the request GET %s to the Server1 with port number "SERVER1_PORT" and IP address %s.\n",client_table[k].key,s);
    ...
    printf("The Client1's port number is %d and IP address is ",ipv4_c->sin_port);
	...
	if((numbytes = recvfrom(sockfd, buf, MAXBUFLEN-1, 0, (struct sockaddr *)&their_addr, &addr_len))==-1){	//receive the value from server1
	   ...
	   exit(1);
        ...
        if(numbytes>0){
	   buf[numbytes]='\0';
	   printf("The Client 1 received the value %s from the server 1 with port number "SERVER1_PORT" and IP address %s.\n",buf,s);
	   printf("The Client1's port number is %d and IP address is ",ipv4_c->sin_port);
	   for(j=0;addr_list[j]!=NULL;j++)
		printf("%s.\n",inet_ntoa(*addr_list[j]));
	   break;
